...
public class Baekjoon14889 {
  ...
  private static void combi(int index, int count) {     // index = 인덱스, count = 조합 개수(=재귀 깊이)
    if (count == n / 2) {
      diff();   // start와 link의 능력치 차이 계산
      return;
    }
    ...
    for (int i = index; i < n; i++) {
      if (!visited[i]) {
        visited[i] = true;
        combi(i + 1, count + 1);    // dfs 실행
        visited[i] = false;
      }
    }
  }

  private static void diff() {      // start와 link의 능력치 차이 계산
    int start = 0;
    int link = 0;
