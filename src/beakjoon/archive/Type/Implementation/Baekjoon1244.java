...
- 1부터 연속적으로 번호가 붙어있는 스위치들이 있다. 
  ...
  3번 스위치를 중심으로 2번, 4번 스위치의 상태가 같고 1번, 5번 스위치의 상태가 같으므로, 
...
public class Baekjoon1244 {
  public static void main(String[] agrs) throws IOException {
    ...
    StringTokenizer st = new StringTokenizer(br.readLine());    // 스위치 상태
    int[] arr = new int[n + 1];     // 스위치 상태 저장
    ...
    for (int i = 0; i < m; i++) {
      ...
      gender = Integer.parseInt(st.nextToken());    // 남: 1, 여: 2
      ...
      if (gender == 1) {
        ...
        int range = Math.min(number - 1, n - number);
        for (int j = 1; j <= range; j++) {
          if (arr[number + j] == arr[range - j]) {
