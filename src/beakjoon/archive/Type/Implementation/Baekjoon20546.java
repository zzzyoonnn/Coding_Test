package beakjoon.archive.Type.Implementation;
/*
문제
- "오늘도 호재만 있게 해주세요. 버핏-"
...
public class Baekjoon20546 {
  public static void main(String[] agrs) throws IOException {
    ...
    for (int i = 0; i < 14; i++) {
      if (j_money / stock[i] >= 1) {
        int temp = j_money / stock[i];
        j_money -= temp * stock[i];
        j_stock += temp;
      }
    }
    j_total = j_money + (j_stock * stock[13]);
    ...
    for (int i = 1; i < 14; i++) {
      if (stock[i - 1] > stock[i]) {
        ...
        u_count = 0;
        if (d_count >= 3) {
          int temp = s_money / stock[i];
          s_money -= temp * stock[i];
          s_stock += temp;
        }
      }
    ...
    s_total = s_money + (s_stock * stock[13]);

    if (j_total > s_total) System.out.println("BNP");
    else if (j_total < s_total) System.out.println("TIMING");
