package beakjoon.archive.algorithm.Set_Map_By_Hashing;
/*
문제
- 이 대회의 채점 프로그램은 각 팀별로 시간 순서대로 쌓인 제출 로그를 분석하여 결과를 낸다. 
  ...
  대회 시각 3분째에 첫 제출로 문제 E를 해결했으며, 
  문제 A에 대해서는 세 번을 제출하여 200분째에 정답을 받았고, 
  문제 D는 300분째에 한 번의 제출로 해결하였다. 
  위 로그를 기반으로 산정한 페널티는 543이 되는데, 
  이는 E를 해결한 시각 3과 A를 해결한 시각 200, 
  A를 두 번 틀린 페널티 40, D를 해결한 시각 300을 더한 값이다. 
  ...
  하지만 제출 자체는 시간 순으로 이루어졌으므로, 
  같은 시각에 같은 문제를 제출한 로그가 남아 있다면 마지막 제출을 제외한 모든 제출은 오답이었을 것이다.
...
- 로그가 n줄에 걸쳐 주어진다. (0 ≤ n ≤ 100)
  각 로그는 공백으로 구분된 세 파트로 이루어지며, 정수 m, 문제 이름, 결과를 의미한다. 
  (0 ≤ m ≤ 300, 문제 이름은 알파벳 대문자, 결과는 "right" 또는 "wrong")
...
public class Baekjoon11531 {
  public static void main(String[] agrs) throws IOException {
    ...
    while (true) {
      ...
      int time = Integer.parseInt(st.nextToken());  // 로그 제출 시각
      String question = st.nextToken();
      String result = st.nextToken();

      if (map.get(question) != null) {
        map.replace(question, map.get(question) + 1);
      } else {
        map.put(question, 0);
      }
      ...
      if (result.equals("right")) {
        total++;
        penalty += (time + (map.get(question) * 20));
      }
    }
