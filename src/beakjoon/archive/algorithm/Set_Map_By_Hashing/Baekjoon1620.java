...
public class Baekjoon1620 {
  public static void main(String[] agrs) throws IOException {
    ...
    for (int i = 0; i < n; i++) {
      ...
      name.put(poketMon, Integer.toString(i + 1));
      num.put(Integer.toString(i + 1), poketMon);
    }

    for (int j = 0; j < m; j++) {
      String s = br.readLine();

      if (name.containsKey(s)) sb.append(name.get(s)).append('\n');
      else if (num.containsKey(s)) sb.append(num.get(s)).append('\n');
    }
