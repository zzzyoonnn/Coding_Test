...
public class Baekjoon24482 {
  ...
  public static void main(String[] agrs) throws IOException {
    ...
    for (int i = 1; i <= n; i++) Collections.sort(list[i], Collections.reverseOrder());     // 인접 정점을 내림차순으로 방문
    ...
    count = 0;
    depth = new int[n + 1];
    for (int i = 0; i <= n; i++) depth[i] = -1;     // 방문되지 않은 노드의 깊이가 -1이기에, 초기값을 -1로 설정
    depth[r] = 0;       // 시작 정점의 깊이 0
    dfs(r, visited, 1);

    for (int i = 1; i <= n; i++) System.out.println(depth[i]);
  }

  private static void dfs(int index, boolean[] visited, int count) {
