...
- ‘쩰리’는 점프하는 것을 좋아하는 젤리다. 
  ...
  새로운 점프 게임의 조건은 다음과 같다.
    ...
    2. ‘쩰리’의 출발점은 항상 정사각형의 가장 왼쪽, 가장 위의 칸이다. 
       다른 출발점에서는 출발하지 않는다.
    2. ‘쩰리’가 이동 가능한 방향은 오른쪽과 아래 뿐이다. 
       위쪽과 왼쪽으로는 이동할 수 없다.
    3. ‘쩰리’가 가장 오른쪽, 가장 아래 칸에 도달하는 순간, 그 즉시 ‘쩰리’의 승리로 게임은 종료된다.
    4. ‘쩰리’가 한 번에 이동할 수 있는 칸의 수는, 현재 밟고 있는 칸에 쓰여 있는 수 만큼이다. 
...
public class Baekjoon16173 {
  ...
  private static void dfs(int i, int j) {
    ...
    for (int k = 0; k < 2; k++) {   // 가능한 이동 방향(오른쪽, 아래)을 반복
      int x = i + dx[k] * arr[i][j];
      int y = j + dy[k] * arr[i][j];
      if (x >= n || y >= n || visited[x][y]) continue;  // 이동이 행렬 범위 내에 있고 셀이 방문되지 않았는지 확인
