...
public class Baekjoon3613 {
  public static void main(String[] agrs) throws IOException {
    ...
    // 시작이 _이거나 마지막이 _ 불가
    if (st.charAt(0) == '_' || st.charAt(st.length() - 1) == '_' ) {
    ...
    boolean upperCase = false;  // 만약 대문자가 있다면 c++로만 변경
    boolean toJava = false; // true 라면 자바, false라면 c++
    ...
    if (toJava) {   // 자바로 변경
      boolean nextUpper = false;    // true 라면 대문자 나와야 함
      for (int i = 0; i < st.length(); i++) {
        char c = st.charAt(i);
        if (nextUpper) {
          sb.append(String.valueOf(c).toUpperCase());   // 대문자로 변경
          nextUpper = false;
        } else {
          if (st.charAt(i) == '_') nextUpper = true;
          else sb.append(c);
        }
      }
    }
