...
- 팀 레드시프트는 대회 준비를 하다가 지루해져서 샌드박스 게임인 ‘마인크래프트’를 켰다. 
  마인크래프트는 1 × 1 × 1(세로, 가로, 높이) 크기의 블록들로 
  이루어진 3차원 세계에서 자유롭게 땅을 파거나 집을 지을 수 있는 게임이다.
  ...
  public static void main(String[] agrs) throws IOException {
    ...
    for (int x = min; x <= max; x++) {
      ...
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
          if (arr[i][j] < x) {
            block = block - (x - arr[i][j]);
            time = time + x - arr[i][j];
          } else if (arr[i][j] > x) {
            time = time + (2 * (arr[i][j] - x));
            block = block + arr[i][j] - x;
          }
        }
      }
      if (block < 0) break;
    ...
    System.out.println(total_time + " " + height);
